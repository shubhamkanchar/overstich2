...
const seller = () => {
    ...
    if($('#warehouseForm').length > 0){
        $(document).on('click','#returnAddress',function(){
            $('.return-stage').toggleClass('d-none');
            if($('.return-stage').hasClass('d-none')){
                $('.return-input').attr('required',false);
            }else{
                $('.return-input').attr('required',true);
            }
        })
    }
