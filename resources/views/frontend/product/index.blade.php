        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb fs-5">
                ...
                @if ($category->masterCategory)
                    <li class="breadcrumb-item"><a href="{{ route('products.index', $category->masterCategory->id) }}">{{ $category->masterCategory->category }}</a></li>
                @endif
                @if ($category->parentSubCategory)
                    <li class="breadcrumb-item"><a href="{{ route('products.index', $category->parentSubCategory->id) }}">{{ $category->parentSubCategory->category }}</a></li>
                @endif
        ...
        <div class="row mb-5 justify-content-between row-gap-5">
            <div class="col-md-3 col-12 filters d-none d-md-block">
                <div class="card bg-white">
                    ...
                    <div class="accordion m-2" id="accordionExample">
                        @foreach ($category->filters as $filter)
                            <div class="accordion-item">
                                ...
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{$filter->id}}" aria-expanded="false" aria-controls="collapse{{$filter->id}}">
                                    {{ $filter->type }}
                                </button>
                                </h2>
                                <div id="collapse{{$filter->id}}" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                                <div class="accordion-body">
                                    ...
                                    @foreach (json_decode($filter->value) as $value)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" multiple type="checkbox" @checked(!empty($currentFilters) && isset($currentFilters[$value])) name="{{$filter->type}}[{{$value}}]" value="{{$value}}">
                    ...
                    <div class="card-body">
                        <button type="submit" class="btn btn-dark ml-2 w-100" style="border-radius: 0%">Apply Filters</button>
                    </div>
                </div>
            </div>
                ...
                @if (count($products) == 0)
                    ...
                    <div class="mb-3 d-flex align-items-center justify-content-between translate-y-up">
                        <div class="col-md-10 col-8 position-relative">
                            <input type="text" name="search" value="{{ $search }}" class="form-control" placeholder="Search Your Product Here" style="border-radius: 0%">
                            @if (!empty($search))
